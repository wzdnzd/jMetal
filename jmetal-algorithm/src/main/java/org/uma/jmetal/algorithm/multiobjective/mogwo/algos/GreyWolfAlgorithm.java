...
public class GreyWolfAlgorithm<S extends Solution<?>> implements Algorithm<List<WolfSolution>> {
    // 当前迭代次数
    ...
    public GreyWolfAlgorithm(int maxEvaluations, int wolvesNum, PositionInitializer positionInitializer,
    ...
    public void run() {
        ...
        List<WolfSolution> candidates = new ArrayList<>();
        List<WolfSolution> reserves = new ArrayList<>();
        while (!isStoppingConditionReached()) {
            ...
            for (int i = 0; i < wolvesNum; i++) {
                ...
                if (archive.size() > 1) {
                    for (WolfSolution wolfSolution : archive) {
                        if (!deltaWolf.equals(wolfSolution)) {
                            candidates.add(wolfSolution);
                        }
                    }
                    betaWolf = leaderSelector.select(candidates);
                }

                if (archive.size() > 2) {
                    for (WolfSolution wolfSolution : candidates) {
                        if (!betaWolf.equals(wolfSolution)) {
                            reserves.add(wolfSolution);
                        }
                    }
                    alphaWolf = leaderSelector.select(reserves);
                }
